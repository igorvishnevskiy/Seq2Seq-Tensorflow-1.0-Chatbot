...
_steps_limit = flags.get_steps_limit()

_save_logs = flags.save_logs_bool
...
def train():
  ...
  with tf.Session() as sess:
    ...
    if _save_logs:
        summary_writer = tf.summary.FileWriter("logs/", sess.graph) #tf.train.SummaryWriter("logs/", sess.graph_def)

    #while True:
    for itteration in range(_steps_limit):
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        checkpoint_path = os.path.join(FLAGS.train_dir, "tf_bot.ckpt")
        model.saver.save(sess, checkpoint_path, global_step=model.global_step, write_meta_graph=True)
        step_time, loss = 0.0, 0.0
